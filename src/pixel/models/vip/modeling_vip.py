...
from .configuration_vip import VIPConfig

logger = logging.getLogger(__name__)


VIP_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "Team-PIXEL/pixel-base",
]


class VIPForBiaffineParsing(ViTForImageClassification):
...
class VIPForTokenClassification(ViTForImageClassification):
    def __init__(self, config):
        super().__init__(config)
...
class VIPForQuestionAnswering(ViTForImageClassification):
    ...
    def forward(
        ...
        outputs = self.vit(
            pixel_txt_values=pixel_txt_values,
            pixel_img_values=pixel_img_values,
        ...
        if attention_txt_mask is None and attention_img_mask is None:
            attention_txt_mask = torch.ones((pixel_txt_values.shape[0], self.vit.embeddings.num_txt_patches), device=self.device)
            attention_img_mask = torch.ones((pixel_img_values.shape[0], self.vit.embeddings.num_img_patches), device=self.device)
        batch_size, num_channels, _, _ = pixel_txt_values.shape
        attention_mask = torch.cat((attention_txt_mask, 
...
class VIPPatchEmbeddings(nn.Module):
    """
...
class VIPEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.patch_img_embeddings = VIPPatchEmbeddings(
            image_size=config.image_size,
            patch_size=config.patch_size,
            num_channels=config.num_channels,
            embed_dim=config.hidden_size,
        )
        self.patch_txt_embeddings = VIPPatchEmbeddings(
            image_size=config.text_size,
            patch_size=config.patch_size,
            num_channels=config.num_channels,
            embed_dim=config.hidden_size,
        )
        ...
        self.position_txt_embeddings = nn.Parameter(
            torch.zeros(1, self.num_txt_patches + 1, config.hidden_size), requires_grad=False
        )
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_2d_sincos_pos_embed(
            self.position_txt_embeddings.shape[-1], int(self.patch_txt_embeddings.num_patches ** 0.5), add_cls_token=True
        )
        self.position_txt_embeddings.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))

        # initialize patch_embeddings like nn.Linear (instead of nn.Conv2d)
        w = self.patch_txt_embeddings.projection.weight.data
        torch.nn.init.xavier_uniform_(w.view([w.shape[0], -1]))

        # timm's trunc_normal_(std=.02) is effectively normal_(std=0.02) as cutoff is too big (2.)
        torch.nn.init.normal_(self.cls_txt_token, std=self.config.initializer_range)
    ...
    def forward(self, pixel_txt_values, pixel_img_values, attention_txt_mask=None, attention_img_mask=None, patch_mask=None):
        batch_size, num_channels, height, width = pixel_txt_values.shape
        txt_embeddings = self.patch_txt_embeddings(pixel_txt_values)
        img_embeddings = self.patch_img_embeddings(pixel_img_values)

        # add position embeddings w/o cls token
        txt_embeddings = txt_embeddings + self.position_txt_embeddings[:, 1:, :]
        img_embeddings = img_embeddings + self.position_img_embeddings[:, 1:, :]
        ...
        # append cls token
        cls_token = self.cls_token + self.position_txt_embeddings[:, :1, :]
...
class VIPSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in VIPModel forward() function)
...
class VIPSelfOutput(nn.Module):
    """
    The residual connection is defined in VIPLayer instead of here (as is the case with other models), due to the
...
class VIPAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = VIPSelfAttention(config)
        self.output = VIPSelfOutput(config)
...
class VIPIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class VIPLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.seq_len_dim = 1
        self.attention = VIPAttention(config)
        self.intermediate = VIPIntermediate(config)
        self.output = VIPOutput(config)
        self.layernorm_before = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.layernorm_after = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)

    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False):
        self_attention_outputs = self.attention(
            self.layernorm_before(hidden_states),  # in VIP, layernorm is applied before self-attention
            attention_mask,
            head_mask,
            output_attentions=output_attentions,
        )
        ...
        # in VIP, layernorm is also applied after self-attention
...
class VIPEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([VIPLayer(config) for _ in range(config.num_hidden_layers)])
...
class VIPPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = VIPConfig
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, VIPEncoder):
            module.gradient_checkpointing = value


class VIPModel(VIPPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = VIPEmbeddings(config)
        self.encoder = VIPEncoder(config)
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = torch.ones((pixel_txt_values.shape[0]+pixel_img_values.shape[0], self.embeddings.num_txt_patches+self.embeddings.num_img_patches), device=self.device)

        embedding_output, attention_mask, mask, ids_restore = self.embeddings(pixel_txt_values, pixel_img_values, attention_mask, patch_mask)
...
class VIPDecoder(nn.Module):
    def __init__(self, config, num_patches, dtype):
        ...
        self.decoder_layers = nn.ModuleList(
            [VIPLayer(decoder_config) for _ in range(config.decoder_num_hidden_layers)]
        )
...
class VIPForPreTraining(VIPPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.vit = VIPModel(config)
        self.decoder = VIPDecoder(config, num_patches=self.vit.embeddings.num_patches, dtype=self.vit.dtype)
    ...
    def forward(
        ...
        return VIPForPreTrainingOutput(
            loss=loss,
            logits=logits,
            mask=mask,
            attention_mask=attention_mask,
...
@dataclass
class VIPForPreTrainingOutput(ModelOutput):
    """
    Class for VIPForPreTraining's outputs, with potential hidden states and attentions.
...
@dataclass
class VIPModelOutput(ModelOutput):
    """
    Class for VIPModel's outputs, with potential hidden states and attentions.
...
@dataclass
class VIPDecoderOutput(ModelOutput):
    """
    Class for VIPDecoder's outputs, with potential hidden states and attentions.
